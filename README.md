...
- [ ] More re-usable and maintainable styles
  ...
  - [ ] Maybe we should style the html components at the theme level? Something like `className="text-blue-600 hover:underline"` on the <a> tags?
  - [ ] the tailwind classes are very specific, I think in general we should have global styles that should deal with 90% of the styling and only add specific styling to elements when needed. I should research this and come up with a plan to implement. Like the font-sans font should just be applied everywhere if that's the font I want to go with. I also think all colours I use in my app should be defined at the index.css theme level and re-used. So if any colours are defined else where they should be added here. What is the base layer in tailwind, how best can I utilise it to make styling global and avoid having to style every element individually? Like Should I have a @layer components {}?
  - [ ] Do we need so many fonts? Do we use them all? I'm thinking we should only really have one standard font throughout the app
  - [ ] @AppNavigation.tsx#L11 I kind of miss named components, makes it much easier to read and reason about the code (i.e when I used styled components this was called <AppNavigationContainer>). Or things like this were useful: const TileContainer = styled.div`width: 500px; display: flex; flex-flow: row wrap; justify-content: space-evenly; align-content: space-between; margin-top: 25px; margin-bottom: 50px;`;
  - [ ] Maybe I should start using ids to make the UI html more navigatable, or would it be better to use aria tags?
  - [ ] We should test the <LoadingSpinner /> since as far as I can see everything loads all at once, so I'm not sure we'll ever see this spinner?
  - [ ] apps/client/src/pages/Home/homeData.tsx Can all of this data be returned by the backend instead of having it hardcoded in the frontend? I'm not sure where the integrations object should live but I'm not sure it's here. Overall I would like to delete the homeData.tsx page as extensionCards data should come from the backend and integrations should live elsewhere.
