...
import { IntegrationPill } from "../../components/home/IntegrationPill";
import { extensionCards, integrations } from "./homeData";

export const Home = () => {
  const spotlightCards = extensionCards.filter((card) => card.spotlight);
  const remainingCards = extensionCards.filter((card) => !card.spotlight);
          ...
          <section>
            <div className="grid gap-6 xl:grid-cols-3">
              {spotlightCards.map((card) => (
                <DirectoryCard key={card.name} card={card} />
              ))}
            </div>
          </section>
